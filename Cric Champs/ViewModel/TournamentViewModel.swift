...
enum Url: String {
    case createTournamentUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/tournament/create"
    case createTeamUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/team/create"
    case getTeamsUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/team/view-all"
    case editPlayerUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/player/edit"
    case addPlayerUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/player/register"
    case getPlayersUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/player/view-all"
    case overUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/tournament/set-overs"
    case addGroundUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/ground/add"
    case getGroundUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/ground/view-all?pageSize=10&pageNumber=1"
    case addUmpireUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/umpire/add"
    case getUmpireUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/umpire/view-all?pageSize=2&pageNumber=1"
    case overViewUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/tournament/view"
    case generateFixtureUrl = "http://cric-env.eba-esrqeiw3.ap-south-1.elasticbeanstalk.com/fixture/generate"
}
...
class TournamentViewModel {
    ...
    func createTournament(completion: @escaping((Tournament?, Error?) -> Void)) {
        ...
        networkManger.postData(url: Url.createTournamentUrl.rawValue, parameters: parameterForCreateTournament, headers: header, image: tournamentLogo, imageFieldName: "logo") { (response, data, error) in
            if error != nil {
            ...
            if response.statusCode == 200 {
                if let data = data as? [String: Any]{
                    ...
                    self.homeVM.currentTournamentId = tournament.id
    ...
    func saveTeam(completion: @escaping((Bool, Error?) -> Void)) {
        ...
        let networkManger = NetworkManager()
        networkManger.postData(url: Url.createTeamUrl.rawValue, parameters: parameterForAddteam, headers: header, image: teamLogo, imageFieldName: "teamPhoto" ) { (response, data, error) in
            if response.statusCode == 200 {
                if let data = data as? [String: Any]{
    ...
    func fetchTeam(completion: @escaping((Bool, Error?) -> Void)) {
        ...
        var allTeams = [Team]()
        networkManager.getTournamentData(url: Url.getTeamsUrl.rawValue, headers: headerForGetTeams) {data, response, error in
            if response == 200 {
                if let data = data {
                    for team in data {
    ...
    func savePlayer(completion: @escaping((Bool, Error?) -> Void)) {
        ...
        let networkManger = NetworkManager()
        networkManger.postData(url: Url.addPlayerUrl.rawValue, parameters: parametersForSavePlayer, headers: header, image: playerProfile, imageFieldName: "playerPhoto" ) { (response, data, error) in
            if response.statusCode == 200 {
                if data != nil {
    ...
    func fetchPlayers(completion: @escaping((Bool, Error?) -> Void)) {
        let headerForPlayers = setHeaderForGetPlayer()
        var allPlayers = [Player]()
        if headerForPlayers["teamId"] == "" {
            self.players = allPlayers
            completion(true, nil)
        } else {
            let networkManager = NetworkManager()
            networkManager.getTournamentData(url: Url.getPlayersUrl.rawValue, headers: headerForPlayers) {data, response, error in
                if response == 200 {
                    if let data = data {
                        for player in data {
    ...
    private func setHeaderForGetPlayer() -> [String: String]{
        var header: [String: String] = [:]
        if let id = tournamentDetails?.id {
            header["tournamentId"] = String(id)
        }
        if let team = currentTeam {
            header["teamId"] = String(team.id)
        } else {
            header["teamId"] = ""
        }
        return header
    ...
    private func fetchPlayerDetails(data: [String: Any]) -> Player {
        ...
        var player = Player(playerId: Int64(0), teamId: Int64(0), name: "", place: "", phone: "", designation: "", expertise: "", batting: "", bowling: "", bowlingType: "", profilePhoto: "", isSaved: true)
    ...
    func setOver(over: Int, completion: @escaping((Bool, Error?) -> Void)) {
        ...
        let networkManger = NetworkManager()
        networkManger.patchData(url: Url.overUrl.rawValue, parameters: ["":""], headers: headerForOver, image: nil) { statusCode, error in
            if statusCode == 200 {
    ...
    func addGround(completion: @escaping((Bool, String?, Error?) -> Void)) {
        ...
        let networkManger = NetworkManager()
        networkManger.postData(url: Url.addGroundUrl.rawValue, parameters: parameterForAddGround, headers: header, image: nil, imageFieldName: "groundPhoto" ) { (response, data, error) in
            if response.statusCode == 200 {
                if let data = data as? [String: Any]{
    ...
    func fetchGrounds(completion: @escaping((Bool, Error?) -> Void)) {
        ...
        let networkManager = NetworkManager()
        networkManager.getTournamentData(url: Url.getGroundUrl.rawValue, headers: headerForGetGrounds) {data, response, error in
            if response == 200 {
                if let data = data {
                    for ground in data {
    ...
    func saveUmpire(completion: @escaping((Bool, Error?) -> Void)) {
        ...
        let networkManger = NetworkManager()
        networkManger.postData(url: Url.addUmpireUrl.rawValue, parameters: parameterForAddUmpire, headers: header, image: umpireProfile, imageFieldName: "profilePhoto" ) { (response, data, error) in
            if response.statusCode == 200 {
                if let data = data {
    ...
    func fetchUmpire(completion: @escaping((Bool, Error?) -> Void)) {
        ...
        networkManager.getTournamentData(url: Url.getUmpireUrl.rawValue, headers: headerForGetGrounds) {data, response, error in
            if response == 200 {
                if let data = data {
                    for element in data {
    ...
    func makeGenerateFixtureRequest(completion: @escaping((Bool, Error?) -> Void)) {
        let generateFixtureHeader = setHeaderForGenerateFixture()
        let networkManager = NetworkManager()
        networkManager.postData(url: Url.generateFixtureUrl.rawValue, parameters: [:], headers: generateFixtureHeader, image: nil, imageFieldName: nil) {response, data, error in
            if response.statusCode == 200 {
    ...
    private func setHeaderForGenerateFixture() -> [String: String] {
        ...
        return generateFixtureHeader
    }
    
    
    func updatePlayerDetail(player: Player, completion: @escaping((Bool, Error?) -> Void)) {
        let networkManager = NetworkManager()
        setHeader()
        //let parameter = setParameterForUpdatePlayerDetail(player: player)
        networkManager.putData(url: Url.editPlayerUrl.rawValue, parameters: [:], headers: header, image: nil, imageFieldName: nil) { response, data, error in

        }
    }
    
    func setParameterForUpdatePlayerDetail(player: Player) -> [String: Any]{
